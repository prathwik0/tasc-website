...
<div class="flex min-h-screen w-full flex-col sm:flex-row">
	...
	<div class="h-2/3 w-full bg-[#0e162a59] px-10 pt-10 sm:h-full sm:w-2/3 md:px-20">
					...
					{#if data.images}
						<div class="grid w-full grid-cols-1 place-items-center gap-10 pb-10 sm:pb-0 md:grid-cols-2 lg:grid-cols-3">
							{#each data.images as i}
								<img src={`/facilities/${i}.JPG`} class="h-44 w-72 rounded-md drop-shadow-2xl md:h-48" alt="" />
							{/each}
						</div>
					{/if}
