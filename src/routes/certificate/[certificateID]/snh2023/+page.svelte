<script lang="ts">
	...
	async function printCertificate() {
		hidden = false;
		setTimeout(() => {
			htmlToImage.toJpeg(certificate, { quality: 0.95 }).then(function (dataUrl) {
...
<h1 class="pt-10 text-center text-4xl drop-shadow-[0_0_.3rem_#ffffff] animate-pulse">Issued Certificate</h1>
<div class="mx-2 mt-6 flex min-h-screen flex-col items-center justify-center">
	<div class="relative z-10 flex w-full flex-col items-center border-8 border-double border-purple-600 bg-white px-20 py-10 text-center text-black md:py-16 lg:w-2/3">
		...
		<div class="mt-2 max-w-lg text-sm italic sm:text-base md:text-lg">Individual was a <span class="font-medium">{data.role}</span>, and their efforts are appreciated through this certificate</div>
...
<div class="overflow-x-hidden">
	<div class:hidden bind:this={certificate} class="relative z-10 flex w-[1024px] flex-col items-center border-8 border-double border-purple-600 bg-white px-20 py-10 text-center text-black">
		...
		<div class="mt-2 max-w-lg text-lg italic">Individual was a <span class="font-medium">{data.role}</span>, and their efforts are appreciated through this certificate</div>
		<!-- <div class="text-sm sm:text-base">{data.userID}</div> -->
		<!-- <div class="mt-2 text-sm sm:text-base md:text-lg">Issued by <span class="font-bold">TASC @ 2023</span></div> -->
		<div class="pt-10 text-sm text-slate-950">{data.certificateID}</div>
